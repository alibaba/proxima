...
#ifdef http_client_test_VERSION
#define PROXIMA_BE_VERSION_STRING http_client_test_VERSION
#else
 ...
 protected:
  virtual void SetUp() {
    ...
    config.config_.mutable_common_config()->set_log_directory(
        "./test_proxima_be/log/");
    config.config_.mutable_common_config()->set_protocol("http");
...
TEST_F(HttpClientTest, TestClient) {
  ...
  // Create a client
  ProximaSearchClientPtr client = ProximaSearchClient::Create("HttpClient");
