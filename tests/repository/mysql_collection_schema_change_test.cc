...
////////////////////////////////////////////////////////////////////
class MysqlCollectionSchemaChangeTest2 : public ::testing::Test {
 protected:
  MysqlCollectionSchemaChangeTest2() {}
  ~MysqlCollectionSchemaChangeTest2() {}
  void SetUp() {
...
TEST_F(MysqlCollectionSchemaChangeTest2, TestGeneral) {
  brpc::Server server_;
  ...
  ASSERT_EQ(0, server_.Start(PORT, &options));
  {
    ...
    EXPECT_CALL(*mysql_handler, get_fields_meta(_))
        .WillOnce(Invoke([](proto::WriteRequest::RowMeta *meta) -> int {
        ...
        .WillOnce(Invoke([](proto::WriteRequest::RowMeta *meta) -> int {
          meta->add_index_column_metas()->set_column_name("new-index1");
          meta->add_index_column_metas()->set_column_name("new-index2");
          meta->add_forward_column_names("new-forward1");
          meta->add_forward_column_names("new-forward2");
          return 0;
        }))
        .WillRepeatedly(Return(0))
        .RetiresOnSaturation();
    ...
    ASSERT_EQ(index_column_metas[0].column_name(), "new-index1");
    ASSERT_EQ(index_column_metas[1].column_name(), "new-index2");
    ASSERT_EQ(meta.index_column_metas_size(), 2);
    forward_column_names = meta.forward_column_names();
    ASSERT_EQ(forward_column_names[0], "new-forward1");
    ASSERT_EQ(forward_column_names[1], "new-forward2");
    // exit
    collection->stop();
    sleep(2);
