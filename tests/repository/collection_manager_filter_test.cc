...
class CollectionManagerFilterTest : public ::testing::Test {
 protected:
  CollectionManagerFilterTest() {}
...
TEST_F(CollectionManagerFilterTest, TestDrop) {
  brpc::Server server_;
  ...
  ASSERT_EQ(0, server_.Start(PORT, &options));
  {
    ...
    EXPECT_EQ(0, old_collections.size());
    EXPECT_EQ(3, expired_collections.size());
    EXPECT_EQ(
        std::any_of(
            expired_collections.begin(), expired_collections.end(),
            [](const std::string &name) { return name == "collection1-uuid"; }),
        true);
    EXPECT_EQ(
        std::any_of(
            expired_collections.begin(), expired_collections.end(),
            [](const std::string &name) { return name == "collection2-uuid"; }),
        true);
    EXPECT_EQ(
        std::any_of(
            expired_collections.begin(), expired_collections.end(),
            [](const std::string &name) { return name == "collection3-uuid"; }),
        true);
    collection_manager->create_collections(new_collections);
    collection_manager->update_collections(old_collections);
    collection_manager->drop_collections(expired_collections);
    collection_manager->clean_invalid_collections();
    EXPECT_EQ(0, collection_manager->collections_.size());
